...
class ResearchPaperSummarizer:
    def __init__(self, model_name="sshleifer/distilbart-cnn-12-6"):
        ...
        # Load summarization pipeline with proper task name
        try:
            self.summarizer = pipeline(
                "summarization",  # Changed from "summarization" to "summarization" (keep as is)
                model=model_name,
                device=self.device,
                truncation=True
            )
            ...
            # Alternative: load model directly
